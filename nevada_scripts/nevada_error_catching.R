...
find_all_errors <- function(data){
  ...
  alpha_shift <- data %>% 
    ...
    ungroup() %>% 
    mutate(diff_sd = sd(alignfinal-align1)) %>%
    mutate(alpha_shift = case_when(
      alignfinal-align1 > (3*diff_sd) & 
        align1_lag-alignfinal_lag > (3*diff_sd) & 
        viable1_lag ~ lag(candidate), 
      alignfinal-align1 > (3*diff_sd) & 
        align1_lead-alignfinal_lead > (3*diff_sd) & 
        viable1_lead ~ lead(candidate)
    )) %>% 
    group_by(precinct_full) %>% 
    mutate(has_alpha_shift = case_when(
      sum(!is.na(alpha_shift)) > 0 ~ TRUE,
      TRUE ~ FALSE
    ))
  
  alpha_shift_cases <- alpha_shift %>% 
    filter(has_alpha_shift == 1) %>% 
    select(-contains("lag"), -contains("lead"), -diff_sd)
  
  data <- left_join(data, alpha_shift_cases)
...
# 
# 
# ds <- left_join(ds, alpha_shift_cases)
# 
# 
